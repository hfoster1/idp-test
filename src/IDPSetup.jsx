  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f5f5f7", display:"flex", alignItems:"center", justifyContent:"center", padding:"32px 20px", fontFamily:"-apple-system,'SF Pro Display',sans-serif" }}>
      ...
      <div style={{ width:"100%", maxWidth:480 }}>
        ...
        <div style={{ background:"#fff", borderRadius:16, border:"1px solid rgba(0,0,0,0.08)", padding:20, marginBottom:10, boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              ...
              <div style={{ fontSize:11, color:"#86868b", marginTop:2 }}>Prompts for credentials at runtime</div>
            </div>
            <button
              type="button"
        ...
        {/* Credentials */}
        <div style={{ background:"#fff", borderRadius:16, border:"1px solid rgba(0,0,0,0.08)", padding:20, marginBottom:10, boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{ fontSize:11, fontWeight:700, color:"#86868b", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:12 }}>2 — Have These Ready</div>
          <p style={{ fontSize:12, color:"#86868b", margin:"0 0 12px", lineHeight:1.5 }}>The script will prompt for these. Copy each before running.</p>
          <CopyValue label="GitHub token" value={github.token} secret />
...
export default function IDPSetup() {
  const [github, setGithub] = useState({ token:"", org:"", repo:"", branch:"main" });
  const [aws, setAws] = useState({ region:"us-east-1", accessKeyId:"", secretAccessKey:"" });
  ...
  const ready = github.token && github.org && github.repo && aws.accessKeyId && aws.secretAccessKey && pattern;

  const reset = () => {
    setGithub({token:"",org:"",repo:"",branch:"main"});
    setAws({region:"us-east-1",accessKeyId:"",secretAccessKey:""});
    setPattern(null);
    setDone(false);
  };
